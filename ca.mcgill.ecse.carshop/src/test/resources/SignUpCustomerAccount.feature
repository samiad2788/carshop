Feature: Sign up for customer account
  ...
  Scenario: Create a customer account when logged in as a technician
    Given there is an existing username "Tire-Technician"
    Given the user is logged in to an account with username "Tire-Technician"
    When the user provides a new username "User1" and a password "password1"
    Then no new account shall be created
    Then an error message "You must log out of the technician account before creating a customer account" shall be raised
